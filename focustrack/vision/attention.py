...
class AttentionAnalyzer:
    ...
    def _analyze_with_opencv(self, frame: np.ndarray) -> tuple[AttentionMetrics, dict[str, object]]:
        ...
        debug["face_bbox"] = (int(x), int(y), int(x + w), int(y + h))
        face_roi = gray[y : y + h, x : x + w]
        eyes = self.eye_cascade.detectMultiScale(face_roi, scaleFactor=1.1, minNeighbors=6, minSize=(18, 18))
        eye_boxes = []
        for ex, ey, ew, eh in eyes[:2]:
            eye_boxes.append((int(x + ex), int(y + ey), int(ew), int(eh)))
        debug["eye_boxes"] = eye_boxes

        eyes_detected = len(eyes) >= 1
        gaze_direction = "centro" if eyes_detected else "desconocida"
        attention_state = "atento" if eyes_detected else "desviado"
