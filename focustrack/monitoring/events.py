...
def build_timeline_events(history: pd.DataFrame, min_duration_seconds: float = 2.0) -> pd.DataFrame:
    ...
    frame = frame.dropna(subset=["timestamp"]).sort_values(
        ["session_id", "timestamp"] if "session_id" in frame else ["timestamp"]
    )
        ...
        for event_name, (column, positive_values) in EVENT_DEFINITIONS.items():
            ...
            for _, row in session.iterrows():
                ...
                if active and active_start is None:
                    ...
                    events.append(
                        _event_row(session_id, event_name, active_start, last_timestamp or timestamp, details)
                    )
                    active_start = None
                    details = []
            ...
            if active_start is not None and last_timestamp is not None:
                events.append(
                    _event_row(session_id, event_name, active_start, last_timestamp, details)
                )
